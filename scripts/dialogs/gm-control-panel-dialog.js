...
let gmControlPanelDialog = null;

const GM_CONTROL_PANEL_DEFAULT_SIZE = Object.freeze({
  width: 1480,
  height: 940
});
...
function getGmControlPanelInitialSize() {
  const viewportWidth = window?.innerWidth ?? GM_CONTROL_PANEL_DEFAULT_SIZE.width;
  const viewportHeight = window?.innerHeight ?? GM_CONTROL_PANEL_DEFAULT_SIZE.height;

  return {
    width: Math.max(960, Math.min(GM_CONTROL_PANEL_DEFAULT_SIZE.width, viewportWidth - 64)),
    height: Math.max(720, Math.min(GM_CONTROL_PANEL_DEFAULT_SIZE.height, viewportHeight - 96))
  };
}
...
function renderControlPanelMarkup(state) {
  const latestSession = getLatestSession(state);
  const hasLiveSessionClass = latestSession && !isSessionClosed(latestSession)
    ? " wildharvest-gm-panel--has-live-session"
    : "";

  return `
    <div class="wildharvest-dialog wildharvest-gm-panel${hasLiveSessionClass}">
...
function refreshControlPanel(dialog, state, { focusSelector = "" } = {}) {
  ensureStateSelections(state);
  state.lastRefreshedAt = Date.now();
  const root = dialog.element?.querySelector?.("[data-gm-control-root]");
  if (!root) return;
  root.innerHTML = renderControlPanelMarkup(state);
  linkFormLabels(root, "gm-control");
  if (focusSelector) focusDialogControl(dialog, focusSelector);
}
...
function attachControlPanelListeners(dialog, state) {
  dialog.element?.addEventListener("keydown", (event) => {
    ...
    refreshControlPanel(dialog, state, {
      focusSelector: `[data-gm-tab="${nextTabId}"]`
    });
  });
  ...
  dialog.element?.addEventListener("click", async (event) => {
    ...
    if (tabId) {
      ...
      refreshControlPanel(dialog, state, {
        focusSelector: `[data-gm-tab="${tabId}"]`
      });
      return;
    }
    ...
    if (action === "gm-select-session") {
      state.selectedSessionId = String(button.dataset.sessionId ?? "");
      state.activeTab = GM_TABS.RESPONSES;
      refreshControlPanel(dialog, state);
      return;
    }
    ...
    if (action === "gm-live-filter") {
      state.liveFilter = String(button.dataset.filterId ?? GM_LIVE_FILTERS.ALL);
      refreshControlPanel(dialog, state);
      return;
    }

    if (action === "gm-response-filter") {
      state.responseFilter = String(button.dataset.filterId ?? GM_RESPONSE_FILTERS.ALL);
      refreshControlPanel(dialog, state);
      return;
    }

    if (action === "gm-preset-filter") {
      state.presetModeFilter = String(button.dataset.filterId ?? GM_PRESET_FILTERS.ALL);
      refreshControlPanel(dialog, state);
      return;
    }

    if (action === "gm-select-history-actor") {
      state.historyActorId = String(button.dataset.actorId ?? "").trim();
      state.historyEntryKey = "";
      refreshControlPanel(dialog, state);
      return;
    }
    ...
    if (action === "gm-select-history-entry") {
      state.historyEntryKey = String(button.dataset.entryKey ?? "");
      refreshControlPanel(dialog, state);
      return;
    }
    ...
    if (action === "gm-toggle-response") {
      ...
      state.expandedResponseKey = state.expandedResponseKey === entryKey ? "" : entryKey;
      refreshControlPanel(dialog, state);
      return;
    }
    ...
    if (action === "gm-send-preset") {
      ...
      state.selectedSkillOverride = "";
      state.activeTab = GM_TABS.LAUNCH;
      refreshControlPanel(dialog, state);
      return;
    }
  ...
  dialog.element?.addEventListener("change", async (event) => {
    const target = event.target;
    if (!isHtmlElement(target)) return;

    if (target.matches('[name="gmPresetId"]')) {
      state.selectedPresetId = String(target.value ?? "");
      state.selectedSkillOverride = "";
      refreshControlPanel(dialog, state);
      return;
    }
    ...
    if (target.matches('[name="gmSkillOverride"]')) {
      state.selectedSkillOverride = String(target.value ?? "");
      refreshControlPanel(dialog, state);
      return;
    }

    if (target.matches('[name="gmSendMode"]')) {
      state.sendMode = String(target.value ?? GM_SEND_MODES.WHOLE_PARTY);
      refreshControlPanel(dialog, state);
      return;
    }

    if (target.matches('[name="gmPresetFilter"]')) {
      state.presetFilter = String(target.value ?? "");
      refreshControlPanel(dialog, state);
      return;
    }

    if (target.matches('[name="gmHistoryFilter"]')) {
      state.historyFilter = String(target.value ?? "");
      refreshControlPanel(dialog, state);
      return;
    }
...
export function openGmControlPanel() {
  ...
  if (gmControlPanelDialog?.element?.isConnected) {
    gmControlPanelDialog.bringToFront?.();
    return gmControlPanelDialog;
  }
  ...
  const dialog = new DialogV2({
    ...
    content: `<div data-gm-control-root></div>`,
    buttons: [
      {
        action: "close",
        label: t("WILDHARVEST.Dialog.Close")
  ...
  gmControlPanelDialog = dialog;

  let unsubscribe = () => {};
  dialog.addEventListener("render", () => {
    addWindowClasses(dialog, "wildharvest-window--gm");
    dialog.setPosition?.(getGmControlPanelInitialSize());
    refreshControlPanel(dialog, state, {
      focusSelector: `[data-gm-tab="${state.activeTab}"]`
    });
    attachControlPanelListeners(dialog, state);
    unsubscribe = subscribeToSearchSessions((sessions) => {
  ...
  dialog.addEventListener("close", () => {
    unsubscribe?.();
    if (gmControlPanelDialog === dialog) {
      gmControlPanelDialog = null;
    }
  }, { once: true });
