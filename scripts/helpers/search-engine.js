...
import { getDocumentPriceCopper } from "./loot-engine-value-core.js";
...
const compendiumIndexCache = createCompendiumIndexCache(async (packId, pack) => {
  ...
  return normalizeCompendiumIndex(index).map((document) => ({
    packId,
    document,
...
async function getCompendiumDocumentsForPackIds(packIds, emptyErrorMessage = null) {
  ...
  for (const packId of normalizedPackIds) {
    ...
    if (!pack) continue;
    const documents = await compendiumIndexCache.get(packId, pack);
