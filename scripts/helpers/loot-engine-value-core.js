...
function materializeSelectionEntries(selection) {
  const segments = [];
  ...
  for (const segment of segments.reverse()) {
    for (let quantity = 0; quantity < segment.quantity; quantity += 1) {
...
export function selectValueBudgetItems({
  ...
  for (const entry of orderedCandidates) {
    ...
    for (const state of sourceStates) {
      for (let quantity = 1; quantity <= quantityLimit; quantity += 1) {
        const totalCopper = state.totalCopper + (entry.priceCopper * quantity);
        if (totalCopper > upperCopper) break;
        const candidate = {
          totalCopper,
  ...
  best ??= { totalCopper: 0, distinctEntryCount: 0, entryCount: 0, previous: null };
  const entries = materializeSelectionEntries(best);

  return {
    entries,
    totalCopper: best.totalCopper,
    distinctEntryCount: best.distinctEntryCount,
    lowerCopper,
    targetCopper,
    upperCopper,
