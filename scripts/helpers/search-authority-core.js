...
export function normalizePlayerRollPolicy(policy = {}) {
  const requestedMax = Number(policy?.maxExtraModifier ?? MAX_PLAYER_EXTRA_MODIFIER);
  const maxExtraModifier = Math.min(
    MAX_PLAYER_EXTRA_MODIFIER,
    Math.max(0, Number.isFinite(requestedMax) ? Math.trunc(requestedMax) : MAX_PLAYER_EXTRA_MODIFIER)
  );

  return {
    allowExtraModifier: policy?.allowExtraModifier !== false,
    maxExtraModifier,
    allowRollModeSelection: policy?.allowRollModeSelection !== false
  };
}
...
export function claimSearchResolution({
  ...
  const normalizedRollPolicy = normalizePlayerRollPolicy(rollPolicy);
  ...
  if (!actorId) return reject(SEARCH_RESOLUTION_REASONS.ACTOR_REQUIRED);
  if (message?.actorId != null && typeof message.actorId !== "string") {
  ...
  if (!Number.isFinite(extraModifier)
    || Math.abs(extraModifier) > normalizedRollPolicy.maxExtraModifier
    || (!normalizedRollPolicy.allowExtraModifier && extraModifier !== 0)) {
    return reject(SEARCH_RESOLUTION_REASONS.INVALID_EXTRA_MODIFIER);
  }
  if (!ALLOWED_ROLL_MODES.has(rollMode)
    || (!normalizedRollPolicy.allowRollModeSelection && rollMode !== "normal")) {
  ...
  return {
    ...
    request: {
      ...
      extraModifier,
      rollMode,
      rollPolicy: normalizedRollPolicy,
      sessionId,
      senderId
    }
  };
